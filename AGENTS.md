...
This repository is a small Spring Boot service scaffold for `goodgut-server`. It uses Java 21, Maven, Spring Web MVC, DevTools, and Spring Boot's MVC test starter as declared in `pom.xml`.

## Project Structure & Module Organization

- `src/main/java/com/example/goodgut_server/` contains application code. Keep new Java packages under this root package; `HELP.md` notes that `com.example.goodgut-server` is invalid and was normalized to `com.example.goodgut_server`.
- `src/main/resources/` contains runtime configuration, currently `application.properties`.
- `src/test/java/com/example/goodgut_server/` contains JUnit tests. Mirror production package paths when adding tests.
- `.mvn/`, `mvnw`, and `mvnw.cmd` are the Maven wrapper files; prefer them over a system Maven install.

## Build, Test, and Development Commands

- `./mvnw test` runs the test suite on Unix-like shells.
- `.\mvnw.cmd test` runs the test suite from PowerShell or Command Prompt.
- `./mvnw spring-boot:run` starts the service locally with Spring Boot DevTools available at runtime.
- `./mvnw package` compiles, tests, and builds the application artifact.
...
Tests use JUnit 5 with Spring Boot test support. Keep test classes in `src/test/java` with the same package as the code under test, and use `*Tests` for Spring context or integration-style tests, matching `GoodgutServerApplicationTests`. Add focused unit tests for new logic and run `./mvnw test` or `.\mvnw.cmd test` before handing off changes.
...
Do not commit secrets into `application.properties`. Put environment-specific values behind Spring properties or profiles, and document required local values in project docs instead of hard-coding credentials.
