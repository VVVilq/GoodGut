...
describe('profile editor state v2', () => {
  it('keeps taxonomy and custom edits in a draft until accepted', () => {
    const initial=createProfileEditorState({selections:[],customIngredients:[],nutritionThresholds:[]});
  ...
  it('explains a selection covered by an existing subtree',()=>{
    ...
    const withMilk=chooseCatalogueItem(createProfileEditorState({selections:[],customIngredients:[],nutritionThresholds:[]}),MILK,'subtree');
    expect(chooseCatalogueItem(withMilk,goat,'node').consolidationMessage).toBeDefined();
    const withGoat=chooseCatalogueItem(createProfileEditorState({selections:[],customIngredients:[],nutritionThresholds:[]}),goat,'node');
  ...
  it('preserves nutrition thresholds through ingredient draft edits',()=>{
    const threshold={id:'nutrition:sugars',nutrient:'sugars' as const,direction:'above' as const,threshold:5,basis:'per_100g' as const};
    const initial=createProfileEditorState({selections:[],customIngredients:[],nutritionThresholds:[threshold]});
    const edited=addCustom(initial,'one','Inulina');
    expect(edited.draft.nutritionThresholds).toEqual([threshold]);
    expect(edited.draft.nutritionThresholds).not.toBe(initial.active.nutritionThresholds);
  });
});
