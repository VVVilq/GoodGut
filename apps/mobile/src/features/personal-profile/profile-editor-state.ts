...
import { addCustomIngredient, deleteCustomIngredient, ProfileValidationError, removeTaxonomySelection, renameCustomIngredient, selectTaxonomyIngredient } from '@/domain/avoided-ingredients/profile';
import { PersonalProfile } from '@/domain/personal-profile';

export type ProfileEditorState = { active: PersonalProfile; draft: PersonalProfile; fieldErrors: Readonly<Record<string, ProfileValidationError>>; ancestryByNode: Readonly<Record<string, readonly string[]>>; consolidationMessage?: string };
export function createProfileEditorState(active: PersonalProfile): ProfileEditorState { return { active, draft: clone(active), fieldErrors: {}, ancestryByNode: {} }; }
...
function clone(profile:PersonalProfile):PersonalProfile{return {selections:profile.selections.map((item)=>({...item})),customIngredients:profile.customIngredients.map((item)=>({...item})),nutritionThresholds:profile.nutritionThresholds.map((item)=>({...item}))};}
