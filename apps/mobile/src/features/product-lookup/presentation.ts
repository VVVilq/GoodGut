...
function thresholdDetail(rule: { direction: 'above' | 'below'; threshold: number }, fact: { basis: 'per_100g' | 'per_100ml'; unit: 'kcal' | 'g' }): string { return `Twój próg: ${rule.direction === 'above' ? 'powyżej' : 'poniżej'} ${formatNumber(rule.threshold)} ${fact.unit}; produkt: ${fact.basis === 'per_100g' ? '100 g' : '100 ml'}`; }
function unavailableRuleDetail(item: UnavailablePersonalRule): string { if (item.kind === 'ingredient') return `${item.ruleLabel}: ${item.reason === 'partial' ? 'niepełne dane składników' : 'brak danych o składnikach'}`; return `${nutrientLabels[item.rule.nutrient]}: ${{ missing_source: 'brak danych', unknown_basis: 'nieznana podstawa', invalid_value: 'nieprawidłowa wartość', unsupported_unit: 'nieobsługiwana jednostka' }[item.reason]}`; }
...
function sourceErrorTitle(category: 'rate_limited' | 'network_error' | 'invalid_source_response' | 'source_unavailable') { return category === 'rate_limited' ? 'Źródło ograniczyło liczbę zapytań' : 'Dane produktu są chwilowo niedostępne'; }
function sourceErrorDetail(category: 'rate_limited' | 'network_error' | 'invalid_source_response' | 'source_unavailable') { return { rate_limited: 'Odczekaj chwilę i spróbuj ponownie.', network_error: 'Serwer GoodGut nie mógł połączyć się ze źródłem danych.', invalid_source_response: 'Źródło zwróciło dane, których nie można bezpiecznie wyświetlić.', source_unavailable: 'Open Food Facts nie odpowiada prawidłowo. Spróbuj ponownie później.' }[category]; }
function clientErrorTitle(kind: 'missing_configuration' | 'transport_failure' | 'http_error' | 'invalid_response' | 'unexpected') { return kind === 'transport_failure' ? 'Brak połączenia z GoodGut' : 'Nie udało się pobrać produktu'; }
function clientErrorDetail(kind: 'missing_configuration' | 'transport_failure' | 'http_error' | 'invalid_response' | 'unexpected') { return { missing_configuration: 'Adres API nie został skonfigurowany w aplikacji.', transport_failure: 'Sprawdź połączenie z siecią i dostępność serwera GoodGut.', http_error: 'Serwer GoodGut odrzucił zapytanie.', invalid_response: 'Serwer GoodGut zwrócił odpowiedź, której aplikacja nie może bezpiecznie odczytać.', unexpected: 'Wystąpił nieoczekiwany błąd aplikacji.' }[kind]; }
