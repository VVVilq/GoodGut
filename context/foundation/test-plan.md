...
| R-TAX04 | Unresolved structured leaves disappear or accidentally become taxonomy evidence. | API mapper, contract decoder, personal-rules, and presentation tests cover ordered duplicates, yellow unresolved items, exact custom matches, and taxonomy non-matches. | Scan a mixed product and confirm every source position remains visible, unresolved text is yellow, and only an exact custom rule turns it red. | S-03 |
...
| R-NUT02 | Schema-v2 migration loses existing ingredient selections or overwrites its only valid source. | `apps/mobile/src/data/__tests__/personal-profile-repository-test.ts` and `personal-profile-integration-test.ts` prove in-memory migration, legacy-slot preservation, verified v4 save, and exact rule projection. | Upgrade a populated v2 profile, save it, terminate the app, and confirm the same ingredients after restart. | S-04 |
| R-NUT03 | Editing one profile section erases the other section. | `apps/mobile/src/data/__tests__/personal-profile-integration-test.ts` proves mixed-profile round trips, ingredient edits preserving thresholds, nutrition removal preserving ingredients, and v2-to-v4 handoff. | Save from both editors in turn and inspect the complete mixed profile after each restart. | S-04 |
...
| R-NUT05 | Persisted thresholds cannot feed the evaluator with exact direction, identity, and missing-data semantics across product bases. | `apps/mobile/src/data/__tests__/personal-profile-integration-test.ts` round-trips all eight nutrients through repository/store and evaluates persisted equality, missing-value, and cross-basis cases; `personal-rules-test.ts` covers strict above/below boundaries. | Inspect saved rule summaries and confirm the product's reported basis is shown in nutrition results. | S-04 |
...
| R-NUT06 | Combined scan warnings lose counts or turn unavailable nutrition into a zero. | `personal-warning-composition-test.ts` and `personal-warning-integration-test.ts` cover mixed counts, equality across gram and millilitre products, unavailable reasons, and shared wire fixtures. | Exercise the documented fixture server scenarios and record a live physical scan. | S-05 |
