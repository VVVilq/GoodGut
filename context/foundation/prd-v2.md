...
- W przypadku braku produktu lub danych potrzebnych do analizy aplikacja jasno komunikuje, że nie może wiarygodnie ocenić produktu.
...
- **Then** dla produktu odnalezionego po zeskanowanym kodzie widzi Nutri-Score oraz analizę odnoszącą produkt do wybranego schorzenia wraz z uzasadnieniem wyniku

#### Acceptance Criteria

- Gdy zeskanowany kod nie wskazuje produktu dostępnego w zbiorze danych MVP, użytkownik otrzymuje jednoznaczną informację o braku produktu.
...
- Produkty są odnajdywane tylko na podstawie skanu kodu kreskowego; MVP nie oferuje ręcznego wyszukiwania ani przeglądania produktów.
...
- FR-005: Użytkownik może zobaczyć Nutri-Score produktu odnalezionego po zeskanowanym kodzie kreskowym w zbiorze danych przygotowanym dla MVP. Priority: must-have
...
- FR-010: Użytkownik otrzymuje jasną informację, gdy zeskanowany kod kreskowy nie wskazuje produktu dostępnego w zbiorze danych przygotowanym dla MVP. Priority: must-have
  > Socrates: Kontrargument rozważony: niepełne pokrycie produktów może ograniczyć użyteczność podczas zakupów. Rozstrzygnięcie: zachowane; MVP musi jednoznacznie odróżniać brak produktu od braku danych do analizy.
...
- Brak przeglądania i ręcznego wyszukiwania produktów; jedynym wejściem do odnalezienia produktu w MVP jest skan kodu kreskowego.
- Brak cyklicznego odświeżania zbioru danych produktów; MVP korzysta ze zbioru przygotowanego przed testowaniem przepływu.

## Open Questions

1. Jakie minimalne pokrycie produktów przez początkowo przygotowany zbiór danych wystarczy, aby uznać przepływ skanowania za użyteczny? — Owner: user. By: przed walidacją MVP.
2. Czas uzyskania wyniku nie ma jeszcze ustalonego mierzalnego limitu. — Owner: user. By: przed walidacją MVP.
